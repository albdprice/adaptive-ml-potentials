"""
Learning Curves: Plotting Script
=================================

Loads results from learning_curves_compute.py (.npz file) and generates
publication-quality figures.

Usage:
    python learning_curves_plot.py
    python learning_curves_plot.py --input my_results.npz
"""
...
def fit_power_law(x, y):
    """Fit power law to data, return parameters."""
    try:
        log_x = np.log(x)
        log_y = np.log(y)
...
def load_results(path):
    """Load .npz results and reconstruct the dict format."""
    data = np.load(path)
...
def plot_learning_curves(results_extrap, results_interp, save_path='fig_learning_curves.png'):
    ...
    ax.errorbar(train_sizes, mae_direct, yerr=mae_direct_std,
                fmt='o', color=color_direct, linewidth=2, markersize=10,
                capsize=4, label='Direct (learn V)', alpha=0.9)
    ax.errorbar(train_sizes, mae_adaptive, yerr=mae_adaptive_std,
    ...
    ax.errorbar(train_sizes, mae_direct_i, yerr=mae_direct_std_i,
                fmt='o', color=color_direct, linewidth=2, markersize=10,
                capsize=4, label='Direct', alpha=0.9)
    ax.errorbar(train_sizes, mae_adaptive_i, yerr=mae_adaptive_std_i,
                fmt='s', color=color_adaptive, linewidth=2, markersize=10,
...
def create_combined_figure(results_extrap, results_interp, save_path='fig_learning_curves_combined.png'):
    """Create a single combined figure with 3 panels."""
...
def print_summary_table(results_extrap, results_interp):
    ...
    for i, n in enumerate(train_sizes):
        ...
        print(f"{n:<10} {mae_d:.4f} +/- {mae_d_std:.4f}      {mae_a:.4f} +/- {mae_a_std:.4f}      {ratio:.2f}x")
...
def main():
    parser = argparse.ArgumentParser(description='Plot learning curve results from .npz file')
    parser.add_argument('--input', type=str, default='learning_curves_results.npz',
                        help='Input .npz file (default: learning_curves_results.npz)')
    args = parser.parse_args()

    print(f"Loading results from: {args.input}")
    results_extrap, results_interp, n_seeds = load_results(args.input)
    print(f"Loaded results: {len(results_extrap['train_sizes'])} training sizes, {n_seeds} seeds")
